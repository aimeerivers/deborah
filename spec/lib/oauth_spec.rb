...
describe Oauth do
  describe 'getting request token from Google' do
    before do
      Oauth.stub!(:make_request => {'oauth_token' => ['TOKEN'], 'oauth_token_secret' => ['SECRET']})
    end

    it 'makes a request to OAuthGetRequestToken' do
      Oauth.should_receive(:make_request).with('https://www.google.com/accounts/OAuthGetRequestToken', anything, anything)
      Oauth.get_request_token('example.org', 80)
    end

    it 'includes a callback URL' do
      Oauth.should_receive(:make_request).with(anything, hash_including('oauth_callback' => 'http://myhost.com:5839/continue'), anything)
  ...
  describe 'getting access token from Google' do
    ...
    it 'makes a request to OAuthGetAccessToken' do
      Oauth.should_receive(:make_request).with('https://www.google.com/accounts/OAuthGetAccessToken', anything, anything, anything)
    ...
    it 'includes the token and verifier in the request' do
      ...
      Oauth.should_receive(:make_request).with(anything, oauth, anything, anything)
      Oauth.get_access_token(token, verifier, mock)
    end

    it 'signs the request with the secret token' do
      secret = mock(:secret)
      Oauth.should_receive(:make_request).with(anything, anything, anything, secret)
